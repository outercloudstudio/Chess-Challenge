...
namespace ChessChallenge.Application
{
  public class ChallengeController
  {
    ...
    ChessPlayer CreatePlayer(PlayerType type)
    {
      return type switch
      {
        PlayerType.MyBot => new ChessPlayer(new MyBot() { Weights = File.ReadAllText("D:\\Chess-Challenge\\Chess-Challenge\\src\\Training\\Checkpoints\\Checkpoint 10300.txt").Split('\n')[..Trainer.WeightCount].Select(float.Parse).ToArray() }, type, GameDurationMilliseconds),
